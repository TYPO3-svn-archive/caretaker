...
function parse(testCase, source) {
	...
	for (i = 0; i < lines.length; i++) {
		...
		var firstChar = lines[i].replace(/ /, '').substr(0, 1);
		if (firstChar != '-' && firstChar != '#' && firstChar != ':' && firstChar != '@' ) {
			...
			for (additionalLines = i + 1; additionalLines < lines.length; additionalLines++) {
				...
				nextLineFirstChar = lines[additionalLines].replace(/ /, '')
						.substr(0, 1);

				if (nextLineFirstChar != '-' && nextLineFirstChar != ':'
						&& nextLineFirstChar != '#') {
					i = additionalLines - 1;
					break;
				}
				...
				if (nextLineFirstChar == '-') {
					if (paramCount == 1) {
						paramCount++;
						command.value = lines[additionalLines].substr(1)
								.replace(/ /, '');
					}
					if (paramCount == 0) {
						paramCount++;
						command.target = lines[additionalLines].substr(1)
								.replace(/ /, '');
					}
				}
			}
